...
export function initDragging() {
    ...
    const onWinStart = (e) => {
        ...
        if (t.closest("#uie-menu-close").length) return;
        ...
        if (mobile) {
            ...
            else if (t.closest(".phone-status-bar, .uie-phone-drag-handle").length) target = $("#uie-phone-window");
    ...
    const onWinEnd = () => {
        if (windowState.active && windowState.target && windowState.target.length) {
            ...
            if (id === "uie-phone-window" && isDesktop) {
                if (!s.phone) s.phone = {};
