...
import { ensureCraftingCategory, isEnchantComponent, normalizeSlotCategory } from "../craftingCategories.js";
...
function listComponents(s, q) {
  ...
  return (s.inventory.items || []).filter(it => {
    if (!isEnchantComponent(it)) return false;
...
function renderComponents() {
  ...
  list.slice(0, 200).forEach(it => {
    ...
    $row.find(".sub").text(`${normalizeSlotCategory(it)} - ${it.type || "component"} - x${it.qty || 1}`);
...
function parseEnchantMods(parts = []) {
  const out = {};
  const statMap = {
    strength: "str",
    str: "str",
  ...
  for (const part of parts) {
    ...
    const match = text.match(/\b(strength|str|dexterity|dex|intelligence|intellect|int|vitality|constitution|vit|luck|luk|charisma|charm|cha)\b\s*([+-]\s*\d+)/i);
    if (!match) continue;
    const key = statMap[match[1].toLowerCase()];
    const value = Number(String(match[2] || "").replace(/\s+/g, ""));
...
async function applyEnchant() {
  ...
  const enchantMods = parseEnchantMods(fx);
  if (Object.keys(enchantMods).length) {
    const existingMods = base.mods && typeof base.mods === "object" && !Array.isArray(base.mods) ? base.mods : {};
    base.mods = { ...existingMods };
    for (const [key, value] of Object.entries(enchantMods)) {
      base.mods[key] = Number(base.mods[key] || 0) + value;
      if (base.mods[key] === 0) delete base.mods[key];
    }
  }
  if (desc) base.description = desc;
  base.enchant = { name, components: used.map(u => u.name), modsApplied: Object.keys(enchantMods) };
  base.modSource = "enchant";
  base._meta = { ...(base._meta && typeof base._meta === "object" ? base._meta : {}), source: "enchant", updatedAt: Date.now() };
  saveSettings();
