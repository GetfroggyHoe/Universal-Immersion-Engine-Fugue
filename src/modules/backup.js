...
function snapshotState() {
    ...
    if (clone && typeof clone === "object") {
        if (window.UIE_savedStates) clone.savedStates = window.UIE_savedStates;
        if (window.UIE_storySaveSlots) clone.storySaveSlots = window.UIE_storySaveSlots;
    }
...
function stateLooksEmpty(s) {
    try {
        ...
        const hasSavedStates = (s.savedStates && typeof s.savedStates === "object" && Object.keys(s.savedStates).length > 0) || (window.UIE_savedStates && typeof window.UIE_savedStates === "object" && Object.keys(window.UIE_savedStates).length > 0);
        ...
        if (hasSavedStates || hasCalendar || hasMap || hasSocial || hasDiary || hasDatabank || hasDatabankNodes || hasConnections || hasAiPromptProfiles) return false;
        const keys = Object.keys(s).filter(k => !["inventory", "image", "windows", "ui", "currencySymbol", "currencyRate", "connections", "aiPromptProfiles"].includes(k));
...
export async function restoreLatestBackup() {
    ...
    if (target.savedStates) {
        window.UIE_savedStates = target.savedStates;
        delete target.savedStates;
        try { window.UIE_saveSavedStatesToDb?.(window.UIE_savedStates); } catch (_) {}
    }
    if (target.storySaveSlots) {
        window.UIE_storySaveSlots = target.storySaveSlots;
        delete target.storySaveSlots;
