...
const THEMES = {
    ...
    rpg: {
        ...
        vars: {
            ...
            "--cb-border": "rgba(203, 163, 92,0.35)",
            "--cb-accent": "#cba35c",
...
export function resetChatboxRuntime() {
    cache = [];
    renderedCount = 0;
    portalState = null;
    pendingGen = null;
    try { $("#uie-chatbox-list").empty(); } catch (_) {}
}
...
function getMsgFromNode(m) {
    try {
        if (String(m?.dataset?.agent || m?.getAttribute?.("data-agent") || "").trim().toLowerCase() === "system") return null;
        const isUser =
...
function normalizeChatboxTextScale(v) {
    ...
    return Math.max(0.85, Math.min(1.35, n));
}
...
function refreshCache() {
    ...
    if (data) {
        const msgs = data.filter((m) => String(m?.agent ?? m?.role ?? m?.type ?? m?.extra?.agent ?? "").trim().toLowerCase() !== "system" && String(m?.name || "").trim().toLowerCase() !== "system").map((m, idx) => {
...
function portalComposerIntoChatbox() {
    ...
    if (!root) {
        host.innerHTML = `<div style="opacity:0.75; font-weight:900;">Composer not found. (Host DOM changed?)</div>`;
        return;
    }
    portalState = {
        root,
...
export function initChatbox() {
    ...
    $win.off("pointerup.uieChatbox", "#uie-chatbox-gear")
        ...
        const isVisible = $opts.is(":visible");
        $opts.css("display", isVisible ? "none" : "flex");
        ...
        if (!isVisible) {
            ...
            // Hydrate speaker styles
            const defSpeaker = s.ui?.speakerStyles?.default || {};
            $("#uie-sw-speaker-bg-color").val(cssColorToHex(defSpeaker.bg, "#fff1c7"));
            $("#uie-sw-speaker-text-color").val(cssColorToHex(defSpeaker.text, "#3e2723"));
            $("#uie-sw-speaker-accent-color").val(cssColorToHex(defSpeaker.accent, "#8e5431"));
            
            // Populate dialogue font select
            const $fontSelect = $("#uie-sw-speaker-font");
            if ($fontSelect.length) {
                $fontSelect.empty();
                $fontSelect.append($("<option>").val("").text("Default"));
                const fonts = window.UIE_FONT_CHOICES || [];
                fonts.forEach((font) => $fontSelect.append($("<option>").val(font).text(font.split(",")[0])));
                const curFont = defSpeaker.font || "";
                if (curFont && !fonts.includes(curFont)) $fontSelect.append($("<option>").val(curFont).text(curFont));
                $fontSelect.val(curFont);
            }

            // Hydrate launcher settings
            $("#uie-launcher-name").val(s.launcher?.name || "");
            $("#uie-launcher-icon").val(s.launcher?.src || "custom");
            $("#uie-launcher-color").val(cssColorToHex(s.launcher?.color, "#9f4f1f"));
            $("#uie-hide-settings").prop("checked", s.ui?.hide?.settings === true);
        }
    });
    ...
    // Helper to format color settings
    function cssColorToHex(value, fallback = "#fff1c7") {
    ...
    $opts.off("pointerup.uieChatbox", "#uie-chatbox-options-save")
        ...
        // Save speaker styles
        s.ui = s.ui || {};
        s.ui.speakerStyles = s.ui.speakerStyles || {};
        s.ui.speakerStyles.default = {
            ...(s.ui.speakerStyles.default || {}),
            name: "Default",
            bg: String($("#uie-sw-speaker-bg-color").val() || "#fff1c7"),
            text: String($("#uie-sw-speaker-text-color").val() || "#3e2723"),
            accent: String($("#uie-sw-speaker-accent-color").val() || "#8e5431"),
            font: String($("#uie-sw-speaker-font").val() || "")
        };
        
        // Save launcher settings
        s.launcher = s.launcher || {};
        s.launcher.name = String($("#uie-launcher-name").val() || "");
        s.launcher.src = String($("#uie-launcher-icon").val() || "custom");
        s.launcher.color = String($("#uie-launcher-color").val() || "#9f4f1f");
        s.ui.hide = s.ui.hide || {};
        s.ui.hide.settings = $("#uie-hide-settings").prop("checked") === true;

        saveSettings();
        applyTheme();
        try { window.applyCustomCssSettings?.(); } catch (_) {}
        try { window.updateLayout?.(); } catch (_) {}
...
export function openChatbox() {
    ...
    // STOP PROPAGATION on the window to prevent host drawers from opening
