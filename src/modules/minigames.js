...
export class LockPicker {
    ...
    init() {
        ...
        if (!document.getElementById("re-lockpick-modal")) {
            ...
            ring.addEventListener("touchmove", handleRotate, { passive: false });
            ...
            document.getElementById("re-lock-close").addEventListener("click", function() { this.stop(); }.bind(this));
        }
        ...
        // Command Hook
        window.startLockpick = function(options) { return this.start(options); }.bind(this);
    ...
    confirm() {
        ...
        if (dist < 10) {
            ...
            if (typeof this.options?.onFailure === "function") this.options.onFailure();
            setTimeout(function() { this.updateStatus(`SEQUENCE ${this.currentStage + 1}/3`); }.bind(this), 1000);
        }
    }
}
...
export class ScratchCard {
    ...
    init() {
        if (!document.getElementById("re-scratch-modal")) {
            ...
            modal.style.cssText = "display:none; position:fixed; inset:0; background:rgba(3,7,18,0.72); z-index:20000; align-items:center; justify-content:center; flex-direction:column; backdrop-filter:blur(14px) saturate(1.25);";
            modal.innerHTML = `
            ...
            const start = function(e) { return this.scratch(e); }.bind(this);
            const move = function(e) { if (e.buttons === 1 || e.type === "touchmove") this.scratch(e); }.bind(this);

            this.canvas.addEventListener("pointerdown", start);
            this.canvas.addEventListener("pointermove", move);
            this.canvas.addEventListener("touchmove", move, { passive: false });

            document.getElementById("re-scratch-close").addEventListener("click", function() { this.stop(); }.bind(this));
        }

        window.startScratchCard = function(prize) { return this.start(prize); }.bind(this);
...
// ==========================================
// CONTEXTUAL DOM SWITCHES
// ==========================================
import { initDomSwitches, domSwitchManager } from "./domSwitches.js";
export { initDomSwitches, domSwitchManager };
