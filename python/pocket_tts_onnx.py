...
class PocketTTSOnnx:
    ...
    def __init__(
        ...
        lsd_steps: int = 1,
        predefined_voice_dir: Optional[str] = None,
        max_reference_seconds: float = 6.0,
    ):
        ...
        self.max_reference_seconds = max(1.0, min(15.0, float(max_reference_seconds or 6.0)))
        ...
        self.predefined_voice_dir = Path(predefined_voice_dir) if predefined_voice_dir else (
            self.bundle_dir / "embeddings_v3"
        )
        local_predefined_voices = self._local_predefined_voices()
        self.predefined_voices = local_predefined_voices
    ...
    def _local_predefined_voices(self) -> tuple[str, ...]:
        if not self.predefined_voice_dir.is_dir():
            return tuple()
        voices = []
        for metadata_path in self.predefined_voice_dir.glob("*.json"):
            binary_path = metadata_path.with_suffix(".bin")
            if binary_path.exists():
                voices.append(metadata_path.stem)
    ...
    def _load_audio(self, path: Union[str, Path]) -> np.ndarray:
        ...
        max_seconds = max(1.0, min(15.0, float(getattr(self, "max_reference_seconds", 6.0) or 6.0)))
    ...
    def _local_model_state(self, voice_name: str) -> dict[str, dict[str, np.ndarray]]:
        metadata_path = self.predefined_voice_dir / f"{voice_name}.json"
        binary_path = self.predefined_voice_dir / f"{voice_name}.bin"
        if not metadata_path.exists() or not binary_path.exists():
            raise FileNotFoundError(f"Missing local voice state for '{voice_name}'.")

        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        raw = binary_path.read_bytes()
        result: dict[str, dict[str, np.ndarray]] = {}
        for tensor_info in metadata.get("tensors", []):
            source_key = tensor_info.get("source_key") or tensor_info.get("name")
            if not source_key or "/" not in source_key:
                continue
            module_name, tensor_key = source_key.rsplit("/", 1)
            dtype = self._numpy_dtype(tensor_info["dtype"])
            offset = int(tensor_info["offset"])
            nbytes = int(tensor_info["nbytes"])
            shape = tuple(int(item) for item in tensor_info["shape"])
            tensor = np.frombuffer(raw[offset : offset + nbytes], dtype=dtype).reshape(shape).copy()
            result.setdefault(module_name, {})
            result[module_name][tensor_key] = tensor
        return result

    @staticmethod
    def _import_model_state_file(source: Union[str, Path]) -> dict[str, dict[str, np.ndarray]]:
        if not HAS_SAFETENSORS:
            raise ImportError("safetensors is required to load .safetensors voice-state files.")
        result: dict[str, dict[str, np.ndarray]] = {}
    ...
    def prepare_voice_state(self, voice: Union[str, Path, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        if voice_str in self.predefined_voices:
            model_state = self._local_model_state(voice_str)
